...
public class Configuration {
	...
	String uidRootNoTrailer = "1.2.3.4.5.6.7";
	String uidSubroot = "1";
	...
	private void load(File configFile) throws Exception {
		...
		if (configFile.exists()) {
			...
			globalAssigningAuthorityOID = root.getAttribute("globalAssigningAuthorityOID");
			...
			if (uidRoot.endsWith(".")) {
			    int k = uidRoot.length();
			    uidRootNoTrailer = uidRoot.substring(0, k-1);
			} else {
			    uidRootNoTrailer = uidRoot;
			    uidRoot += ".";
			}
	...
	public String getUIDRootNoTrailer() {
		return uidRootNoTrailer;
	}
...
*/
	...
	public String saveElement(String username, Element el) {
		synchronized (this) {
			...
			if (name.equals("config")) {
				...
				globalAssigningAuthorityOID = el.getAttribute("globalAssigningAuthorityOID");
	...
	public String getTable(String filter) {
		...
		sb.append("<tr><td>Global Assigning Authority OID:</td><td>"+globalAssigningAuthorityOID+"</td></tr>");
	...
	public synchronized String getDateTime(int deltaMinutes) {
		calendar.setTimeInMillis(System.currentTimeMillis());
		calendar.add(Calendar.HOUR, deltaMinutes);
		int year = calendar.get(Calendar.YEAR);
		int month = calendar.get(Calendar.MONTH) + 1;
		int day = calendar.get(Calendar.DAY_OF_MONTH);
		int hour = calendar.get(Calendar.HOUR_OF_DAY);
	...
	public Element getServerXML() {
		try {
			...
			e.setAttribute("globalAssigningAuthorityOID", globalAssigningAuthorityOID);
